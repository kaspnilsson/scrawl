...
export const insertTasksIntoUpdate = (
  update: ProjectUpdate,
  tasks: Task[]
): { update: ProjectUpdate; unusedTaskIds: string[] } => {
  ...
  return { update, unusedTaskIds: Object.keys(tasksById) }
}
...
export const insertTasksIntoContent = (
  ...
  tasks: Task[]
): { content: JSONContent; unusedTaskIds: string[] } => {
  const tasksById: { [key: string]: Task } = {}
  for (const u of tasks) tasksById[u.id] = u
  ...
  return { content: out, unusedTaskIds: Object.keys(tasksById) }
}
